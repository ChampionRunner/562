<nav>
        <div class="nav-wrapper blue">
         <!-- SIDENAV -->
        
     

        <a href="#" class="brand-logo center">Smart India Hackathon</a>

        <ul id="slide-out" class="left side-nav">
               			<li><a href="showAllSchools.php">View Schools</a></li>
					<li><a href="viewSchool.php">Filter Schools</a></li>
					<li><a href="viewStud1.php">View Meritorious Students</a></li>
					<li><a href="requestMeri.php">Request Meritorious Students</a></li>
					<li><a href="viewSuggestedStudents.php">View Suggested Students</a></li>
					<li><a href="viewConflicts.php">View Conflicts</a></li>
					
                    <div class="divider"></div>
                </li>
               
            <li><a href="logout.php">Log Out</a></li>
                </ul>
                <a href="#" data-activates="slide-out" class="button-collapse show-on-large"><i class="material-icons">menu</i></a>

       <!-- <ul id="nav-mobile" class="right hide-on-med-and-down">
            <li><a href="addStudent.html">Register Students</a></li>
            <li><a href="#">Log Out</a></li>
        </ul>-->
			</div>
    </nav>
	<script type="text/javascript" src="https://code.jquery.com/jquery-2.1.1.min.js"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/materialize/0.98.2/js/materialize.min.js"></script>

    <script>
       $(".button-collapse").sideNav();

        // Init Sidenav
        $('.button-collapse').sideNav();
    </script>
        </div>
    </nav>
	
	
